...
test('Header has correct text', async function ({ page }) {
    ...
    await checkIfLocatorHasText(h1Locator, testData.expectedTexts.h1)
    await checkIfLocatorHasText(h2Locator, testData.expectedTexts.h2)
    await checkIfLocatorHasText(pLocator, testData.expectedTexts.p)
    await checkIfLocatorHasText(liLocator.first(), testData.expectedTexts.ul[0])
    await checkIfLocatorHasText(liLocator.nth(1), testData.expectedTexts.ul[1])
    await checkIfLocatorHasText(liLocator.nth(2), testData.expectedTexts.ul[2])
});
